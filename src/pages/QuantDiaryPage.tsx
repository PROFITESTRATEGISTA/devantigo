            ...
            <div className="space-y-4">
              {actionType === 'analysis' ? (
                ...
                <div>
                  ...
                  {/* Comentários pré-formatados organizados por categoria */}
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-300 mb-2">Comentários Sugeridos:</h4>
                    <div className="space-y-3 max-h-40 overflow-y-auto">
                      {/* Estratégia */}
                      <div>
                        <h5 className="text-xs font-medium text-blue-400 mb-1">📈 Estratégia</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            ...
                            "Setup de entrada perfeito - replicar amanhã",
                            "Estratégia de scalping eficiente nos primeiros 30min",
                            "Grid trading funcionou bem em mercado lateral",
                            "Breakout falso - aguardar confirmação com volume",
                            "Reversão na resistência - padrão repetitivo identificado",
                            "Divergência no RSI antecipou movimento - excelente sinal",
                            "Cruzamento de médias móveis deu entrada precisa",
                            "Bandas de Bollinger indicaram sobrecompra corretamente"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              ...
                              className="text-xs text-left p-2 bg-gray-700 hover:bg-gray-600 rounded text-gray-300 hover:text-white transition-colors"
                            >
                              {comment}
                            </button>
                          ))}
                      ...
                      {/* Mercado */}
                      <div>
                        <h5 className="text-xs font-medium text-orange-400 mb-1">🌍 Mercado</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            ...
                            "Abertura com gap - aguardei fechamento para operar",
                            "Mercado sem direção definida - dia de observação",
                            "Alta volatilidade no fechamento - evitar últimos 15min",
                            "Correlação entre ativos funcionou como esperado",
                            "Suporte técnico respeitado - nível importante confirmado",
                            "Volume acima da média - movimento sustentado",
                            "Mercado em tendência forte - seguir o fluxo",
                            "Consolidação após movimento forte - aguardar breakout"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Emocional */}
                      <div>
                        <h5 className="text-xs font-medium text-purple-400 mb-1">🧠 Emocional</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            ...
                            "Paciência recompensada - aguardei setup perfeito",
                            "Overtrading por tédio - preciso melhorar autocontrole",
                            "Revenge trading após perda - erro grave cometido",
                            "Confiança alta após sequência de ganhos - cuidado",
                            "Medo de perder ganhos me fez sair muito cedo",
                            "Stress por perda anterior afetou próxima operação",
                            "Mantive calma durante drawdown - mentalidade correta",
                            "Euforia após grande ganho - preciso manter pés no chão"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Técnico */}
                      <div>
                        <h5 className="text-xs font-medium text-green-400 mb-1">📊 Técnico</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            ...
                            "MACD histograma mudou de direção - sinal antecipado",
                            "Fibonacci 61.8% funcionou como suporte exato",
                            "Candlestick doji no topo - reversão confirmada",
                            "Volume profile mostrou área de valor correta",
                            "Pivot points funcionaram como resistência/suporte",
                            "Padrão ombro-cabeça-ombro completado",
                            "Triângulo ascendente rompido com volume",
                            "Bollinger Bands squeeze precedeu movimento forte"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Gestão de Risco */}
                      <div>
                        <h5 className="text-xs font-medium text-red-400 mb-1">⚠️ Gestão de Risco</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            ...
                            "Risk/reward de 1:3 mantido em todas as operações",
                            "Trailing stop protegeu 80% dos ganhos",
                            "Correlação entre posições monitorada - risco controlado",
                            "Hedge com opções funcionou durante volatilidade",
                            "Redução de posição após 2 perdas consecutivas",
                            "Aumento gradual de posição em sequência positiva",
                            "Stop loss mental respeitado - disciplina mantida",
                            "Saída parcial no primeiro alvo - lucro garantido"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Horário */}
                      <div>
                        <h5 className="text-xs font-medium text-yellow-400 mb-1">⏰ Horário</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            "Melhor performance entre 9h30-11h30 - foco neste período",
                            "Evitar trading entre 12h-14h - baixa volatilidade",
                            "Últimos 30min do pregão muito arriscados",
                            "Abertura com gap ofereceu boas oportunidades",
                            "Horário de almoço americano - mercado parado",
                            "Pre-market mostrou direção do dia",
                            "After hours com pouca liquidez - evitar",
                            "Overlap de sessões (Londres/NY) - maior volatilidade"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Resultado */}
                      <div>
                        <h5 className="text-xs font-medium text-cyan-400 mb-1">💰 Resultado</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            "Dia positivo - meta diária atingida com folga",
                            "Perda controlada - dentro do limite de risco diário",
                            "Breakeven - dia neutro, sem grandes emoções",
                            "Melhor dia do mês - estratégia funcionou perfeitamente",
                            "Pior dia da semana - revisar o que deu errado",
                            "Meta semanal atingida em 3 dias - excelente",
                            "Sequência de 5 dias positivos - manter consistência",
                            "Primeira perda após 8 ganhos - normal e esperado"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
                      ...
                      {/* Aprendizado */}
                      <div>
                        <h5 className="text-xs font-medium text-indigo-400 mb-1">🎓 Aprendizado</h5>
                        <div className="grid grid-cols-1 gap-1">
                          {[
                            "Aprendi a importância de aguardar confirmação",
                            "Descobri novo padrão que funciona neste ativo",
                            "Erro custoso me ensinou sobre gestão de risco",
                            "Backtesting confirmou eficácia da nova estratégia",
                            "Análise pós-trade revelou ponto de melhoria",
                            "Estudo de caso interessante para revisão semanal",
                            "Padrão sazonal identificado - anotar para próximo mês",
                            "Correlação com índice americano mais forte que pensava"
                          ].map((comment, index) => (
                            <button
                              key={index}
                              onClick={() => setEditingDay(prev => ({ 
